...
def load_image_bytes(image_url: str) -> tuple[bytes, str] | None:
    ...
    if data_url_match:
        try:
            return base64.b64decode(data_url_match.group(2)), data_url_match.group(1).lower()
        except Exception:
            return None
    if re.fullmatch(r"[A-Za-z0-9+/=\s]+", raw_value) and len(raw_value) > 100:
        try:
            return base64.b64decode(raw_value), _detect_image_mime(base64.b64decode(raw_value))
        except Exception:
            return None
